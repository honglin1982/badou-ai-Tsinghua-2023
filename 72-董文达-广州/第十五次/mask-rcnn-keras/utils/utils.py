...
def minimize_mask(bbox, mask, mini_shape):
    ...
    for i in range(mask.shape[-1]):
        ...
        mini_mask[:, :, i] = np.round(m).astype(np.bool)
    return mini_mask
...
def compute_ap(gt_boxes, gt_class_ids, gt_masks, pred_boxes,
               pred_class_ids, pred_scores, pred_masks, iou_threshold=0.5):
    gt_match, pred_match, overlaps = compute_matches(gt_boxes, gt_class_ids, pred_boxes, pred_class_ids,
...
def compute_ap_range(gt_box, gt_class_id, gt_mask, pred_box, pred_class_id,
    ...
    for iou_threshold in iou_thresholds:
        ap, precesions, recalls, overlaps, =\
        compute_ap(gt_box, gt_class_id, gt_mask,pred_box, pred_class_id,
...
def batch_slice(inputs, graph_fn, batch_size, names=None):
    ...
    for i in range(batch_size):
        ...
        outputs_slice = graph_fn(*inputs_slice)
...
def denorm_boxes(boxes, shape):
    ...
    scale = np.array([h - 1, w - 1, h - 1, w - 1])
    shift = np.array([0, 0, 1, 1])
    return np.around(np.multiply(boxes, scale) + shift).astype(np.float32)
...
def compose_image_meta(image_id, original_image_shape, image_shape, window, scale, active_class_ids):
    meta = np.array(
        [image_id] + list(original_image_shape) + list(image_shape) + list(window) + [scale] + list(active_class_ids)
    )
    return meta
...
def unmold_detections(detections, mrcnn_mask, original_image_shape, image_shape, window):
    ...
    return boxes, class_ids, scale, full_masks
