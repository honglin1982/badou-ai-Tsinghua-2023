#BBoxUtility类下的函数一起提供了对边界框的处理和操作，括计算交并比（IOU）、编码边界框、忽略边界框、指定边界框、解码边界框、检测输出和非极大抑制等。
#这些函数相互调用和拼接，共同构成了边界框处理工具的功能。
import numpy as np
...
class BBoxUtility(object):
    #实现了BBoxUtility类的初始化函数，设置了一些参数
    #初始化了先验框，重叠阈值，忽略阈值，非极大值抑制阈值，top_k等属性
    #使用TensorFlow定义了占位符和非极大值抑制函数
    ...
    #设置和获取非极大值抑制阈值，通过装饰器，将_nms_thresh属性封装为nms_thresh属性，并在设置属性值时更新非极大值抑制函数
    @property
    def nms_thresh(self):
    ...
    def nms_thresh(self, value):
        ...
        self.nms = tf.image.non_max_suppression(self.boxes, self.scores,
                                                self._top_k,
                                                iou_threshold=self._nms_thresh)
    #设置和获取top_k，通过装饰器，将_top_k属性封装为top_k属性，并在设置属性值时更新非极大值抑制函数
    @property
    def top_k(self):
        return self._top_k
